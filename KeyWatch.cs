    ...
    internal static class KeyWatch
    {
        ...
        private static void Run()
        {
            ...
            finally
            {
                ...
                // Останавливать перепись некому: WM_QUIT сюда никто не шлёт. Значит,
                // выход из цикла — это либо −1 от GetMessageW (недействительное окно),
        ...
        /// ведут себя control и Alt. Без поправки галочка у ⌥ в карточке «Особые
        /// клавиши» не загоралась никогда — она спрашивает про 0xA4/0xA5, — а строка
        /// в списке нажатий выглядела как «клавиша код 12 vk=12»: имени у общего кода
        /// нет ни одного. Про control и ⇧ карточка не спрашивает вовсе; поправка нужна
        /// им ради имени в списке.
        /// </summary>
                                       ...
                                       List<int> seen, List<bool> isNew)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                for (int u = 0; u < len; u++)
                {
                    ...
                    // ⏏ за свидетельство не считаем. Признак значит «драйвер Apple
                    // преобразует верхний ряд», а ⏏ шлёт сама алюминиевая клавиатура
                    // по USB — и одного нажатия хватало, чтобы программа уступила ряд
                    // драйверу, который до этой клавиатуры не добрался. F1–F12 после
                    // этого не делали ничего вовсе.
                    if (usage != Native.UsageEject) _mediaSeen = true;
                }
            }
        }
