    ...
    internal sealed class MainWindow : Window
    {
        ...
        private UIElement PageFKeys()
        {
            ...
            if (YieldingNow)
            {
                stack.Children.Add(Card("Сейчас этим занимается драйвер Apple", null,
                    Note("Функциональный ряд обрабатывает родной драйвер Boot Camp, а программа ему " +
                         "уступает — иначе одно нажатие переназначалось бы дважды. Поэтому настройки " +
                         "ниже сейчас ни на что не влияют: F1–F12 до программы просто не доходят, " +
            ...
            if (fm == null || fm.Numpad)
            {
                ...
                stack.Children.Add(Card("Цифровой блок", null, padGrid,
                    Note("Две клавиши цифрового блока Apple в Windows ведут себя не так, как " +
                         ...
                         "было бы нечем.\n\n" +
                         "«=» отдаёт VK_CLEAR со скан-кодом 0x59, которого не понимает почти ни одна " +
                         "программа, поэтому по умолчанию она печатает знак «=» напрямую.")));
            }
            ...
            stack.Children.Add(Card("Яркость", null,
                ...
                Note("Внешние мониторы управляются по DDC/CI, встроенная панель ноутбука — через WMI. " +
                     "Если монитор не отвечает на DDC/CI, эти клавиши ничего не сделают — " +
                     "тогда назначьте на F1 и F2 что-нибудь другое.\n\n" +
                     "Вторая настройка нужна, когда функциональным рядом занимается драйвер. " +
                     "Тогда F1 и F2 до программы не доходят: " +
                     "они переводятся сразу в коды яркости медиастраницы. Windows применяет " +
                     "такие коды только к встроенной панели ноутбука, и на обычном ПК они пропадают " +
                     "зря — а программа ловит их и применяет к внешним мониторам. Так яркость " +
                     "работает даже тогда, когда сам ряд программе не принадлежит.")));
        ...
        private UIElement PageModifiers()
        {
            ...
            stack.Children.Add(Card("Поведение", null,
                ...
                Note("На этой клавиатуре правый ⌥ приходит как AltGr — Windows добавляет к нему " +
                     "невидимый Ctrl, и сочетания вроде ⌥+буква срабатывают не так, как ожидаешь. " +
                     "Выключайте только если не набираете символы через AltGr.")));
        ...
        private UIElement PageDriver()
        {
            ...
            if (installed)
            {
                ...
                modes.Children.Add(PresetButton(
                    ...
                    "F1–F12 сами дают яркость, громкость и перемотку, обычные F-клавиши — через Fn. " +
            ...
            if (installed)
            {
                ...
                remove.Click += delegate
                {
                    ...
                    SetupSay((ok ? "Драйвер удалён." : "Удалить не вышло.") + "\n" + output);
                    AppleDriver.Refresh(true);
                };
        ...
        private void StartSetup(bool install, string readyFolder)
        {
            ...
            _setupThread = new Thread(delegate()
            {
                ...
                catch (Exception e) { SetupSay("Сбой: " + e.Message); }
            });
        ...
        private UIElement PageAbout()
        {
            ...
            stack.Children.Add(Card("Чего она не делает", null,
                ...
                Bullet("Не видит клавишу ⏏: она обрабатывается внутри клавиатуры и наружу " +
                       "не выходит ни по одному каналу — проверено."),
        ...
        private void ApplyDriverProfile()
        {
            ...
            if (AppleDriver.TakesFunctionRow)
            {
                ...
                if (_s.FnNavigation)
                {
                    ...
                    done.Add("• Навигация Fn+стрелки выключена: её тоже берёт на себя драйвер.");
                }
            }
