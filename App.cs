...
namespace MagicKeys
{
    internal sealed class App : Application
    {
        ...
        private int Run(string[] args)
        {
            ...
            // Коды яркости с медиастраницы. Чужой драйвер, забравший верхний ряд, может
            // переводить F1 и F2 сразу в коды 0x70 и 0x6F — тогда до перехвата они
            // не доходят вовсе. Драйвер Apple их не шлёт: замерено, коллекция COL04
            // при нажатии F1 молчит. Ловим здесь ради остальных.
            ...
            // и подсказку значка. Без второго заряд в подсказке не появлялся почти
            // никогда: спрашивает его только сама подсказка да страница «Диагностика»,
            // а по Bluetooth первый ответ приходит уже после того, как подсказку
            // собрали. Запущенная в значок программа показывала заряд лишь после того,
            // как человек тронет любую настройку.
        ...
        private void OnPowerModeChanged(object sender, Microsoft.Win32.PowerModeChangedEventArgs e)
        {
            ...
            // Забыли заряд — значит забыла и подсказка: иначе там продолжает висеть
            // процент, о котором программа уже ничего не знает. Спрашивая его заново,
            // подсказка заодно заводит опрос — а спрашивающих у него всего двое.
            ToWindow(UpdateTrayText);
        }
