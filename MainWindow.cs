...
namespace MagicKeys
{
    internal sealed class MainWindow : Window
    {
        ...
        /// <summary>Итог действия с драйвером уже показан — значит его можно погасить.</summary>
        private bool _shownNotice;
        ...
        private bool BuildPage()
        {
            ...
            {
                _building = false;
                // Отчёты о разовых действиях гасим на выходе, а не на входе: их ставят
                // прямо перед BuildPage, а читает их сама страница уже внутри — гася
                // на входе, мы не показывали ни «Записано», ни «Не вышло», и отказ
                // по правам выглядел точно как успех.
                //
                // Но не после первой же пересборки: страница пересобирается сама —
                // от сменившегося набора устройств, от заряда, от темы Windows.
                // А удаление драйвера набор устройств и меняет, немедленно, — отчёт
                // вместе с требованием перезагрузиться стирался до прочтения. Гасим
                // только те, что показала эта же сборка страницы.
                if (_shownNotice) { _fnNotice = null; _shownNotice = false; }
                _tuneNotice = null;
            ...
            if (!String.IsNullOrEmpty(KeyWatch.Failure))
                lines.Children.Add(Note(KeyWatch.Failure + " " +
                                        ...
                                        (_engine != null && !String.IsNullOrEmpty(_engine.Failure)
                                            ...
                                            : "Переназначения работают, модель клавиатуры " +
                                              "известна, а вот всё, что узнаётся ") +
        ...
        private UIElement PageDriver()
        {
            ...
            if (!String.IsNullOrEmpty(driverSay))
            {
                ...
                stack.Children.Add(Card(_driverBusy ? "Сейчас" : "Последнее действие",
                                        null, Note(driverSay)));
                // Показали — теперь можно и погасить. Гасит выход из BuildPage.
                if (!_driverBusy) _shownNotice = true;
            }
