...
namespace MagicKeys
{
    ...
    internal static class AppleDriver
    {
        ...
        public static bool SetFnBehavior(int value, out string error)
        {
            ...
            // Спрашиваем при любом найденном месте, а не только при ключе службы.
            // Значение могло найтись и у устройства — при установленном, но отключённом
            // драйвере это бывает, — и тогда весь блок пропускался: reg.exe писал
            // в ключ устройства, человек читал «Записано», а карточка над кнопкой
            // в этот самый миг писала «установлен · отключена». Ровно то обещание,
            // которого программа не подтвердит, и ровно тот случай, ради которого
            // отказ ниже и написан.
            if (!String.IsNullOrEmpty(path) && Installed)
            {
                ...
                if (alive == null)
                {
                    // Все службы отключены — записывать некуда: значение читает драйвер,
                    // а он не работает. Подменять службу ключом устройства нельзя по той
                    // же причине.
                    error = "Служба драйвера Apple отключена — записывать некуда. "
                          + "Включите её в «Диспетчере устройств» и повторите.";
                    return false;
                }
                // Найденное у устройства место не трогаем: там драйвер его и держит.
                if (!same && path.IndexOf(@"\Services\", StringComparison.OrdinalIgnoreCase) >= 0)
                    path = alive;
            }
        ...
        public static void Refresh(bool force)
        {
            ...
            try
            {
                ...
                // 8 МБ, 80 мс на четыре имени. Ответ запоминается (см. DeviceKey),
                // так что цена платится один раз за запуск, — но на машине без драйвера
                // он гарантированно пуст, и платить её незачем вовсе.
