...
namespace MagicKeys
{
    internal sealed class MainWindow : Window
    {
        ...
        // Пять щелчков подряд, а не пять за всё время: счёт сбрасывается при уходе
        // со страницы. Иначе пять случайных щелчков в пять разных заходов включали
        // служебный режим сами собой.
        ...
        public void RefreshDevices()
        {
            ...
            string now = all.Count + "|" + apple + "|" + KeyWatch.MaxFunctionKey + "|"
                       ...
                       // И обе беды: их показывает карточка состояния, а без них
                       // отметка не менялась — сорванный перехват до открытого окна
                       // не доходил вовсе и ждал случайной пересборки часами.
                       + "|" + (_engine == null ? "" : _engine.Failure)
                       + "|" + KeyWatch.Failure;
        ...
        private bool BuildPage()
        {
            ...
            _updStatus = null;
            _updAction = null;
            // И счёт щелчков по названию: приём рассчитан на пять подряд, а не на пять
            // случайных за всё время работы программы.
            if (CurrentPage != "about") _aboutClicks = 0;
            ...
            {
                _building = false;
                // Отчёты о разовых действиях живут до ухода со своей страницы.
                //
                // Гасить их на входе нельзя: их ставят прямо перед BuildPage, а читает
                // их сама страница уже внутри — гася на входе, мы не показывали ни
                // «Записано», ни «Не вышло», и отказ по правам выглядел точно как успех.
                // Гасить на выходе — тоже: страница пересобирается сама, от сменившегося
                // набора устройств, от заряда, от темы Windows. Удаление драйвера набор
                // устройств и меняет, немедленно, — отчёт вместе с требованием
                // перезагрузиться исчезал через пару секунд после появления. Признак
                // «уже показали» этого не лечил: обе надписи ставят только на своей же
                // странице, и показывает их та же сборка, что и гасила.
                string page = CurrentPage;
                if (page != "driver") { _fnNotice = null; _tuneNotice = null; }
                if (page != "about") _autostartNotice = null;
            }
            return true;
        }
        ...
        private UIElement PageKeys()
        {
            ...
            if (YieldingNow)
            {
                stack.Children.Add(Card("Сейчас этим занимается драйвер Apple", null,
                    Note("Функциональный ряд обрабатывает родной драйвер Boot Camp, а программа ему " +
                         ...
                         "они приходят уже готовыми медиакодами. " +
                         // Режим гасится только когда клавиш ровно двенадцать: на клавиатуре
                         // с F13–F19 он остаётся живым и решает всё для них. Безусловная
                         // фраза «погашен и выбор режима» отговаривала трогать живой список
                         // — тот самый, которым эти назначения и оживляют.
                         (Models.FunctionKeyCount() > 12
                            ? "Выбор основного режима при этом остаётся: он решает, что делают " +
                              "F13 и дальше, — их драйвер не трогает."
                            : "Вместе с ними погашен и выбор основного режима — он тоже " +
                              "про эти двенадцать клавиш.") +
                         " Остальное на странице работает по-прежнему.\n\n" +
            ...
            stack.Children.Add(Card("Заменитель Fn", null,
                ...
                Note(!_s.FnNavigation
                    ...
                    : _s.FnSubstitute == ModKey.None
                        // Про недоходящую Fn — только когда драйвера нет. С ним она
                        // доходит, и сноска ниже про это же говорит: две фразы в одной
                        // карточке отменяли друг друга.
                        ? (YieldingNow
                            ? "Пока заменителя нет, нажать эти сочетания нечем: навигация Fn " +
                              "держится на заменителе, а не на настоящей Fn."
            ...
            if (KeyWatch.EjectSeen || ejectSet || (fm != null && fm.Eject) || fm == null)
            {
                ...
                AddSingleRow(ejGrid, 0, "Eject", "Верхний правый угол, со значком извлечения",
                    // Пересобираем: сноска под этой же строкой построена на том, есть ли
                    // назначение вовсе. Без пересборки окно продолжало просить выбрать
                    // то, что человек только что выбрал, — и наоборот, обещало работающее
                    // назначение после того, как его сняли.
                    _s.EjectKey, delegate(string id) { _s.EjectKey = id; Save(); BuildPage(); });
                ...
                string ejNote = KeyWatch.EjectSeen && !ejectSet
                    // Назначения нет вовсе — хвалиться нечем, и предупреждать о поломке
                    // настройки, которой человек не задавал, тоже незачем.
                    ? "Эта клавиша с вашей клавиатуры приходит — выберите, что она должна делать."
                    : KeyWatch.EjectSeen
                    ? (live
                    ...
                    : fm != null && fm.Gen == AppleGen.Alu
                        ...
                        : fm == null
                        // Клавиатуры Apple нет вовсе — утверждать что-либо про «эту клавишу»
                        // не о чем. Настройка при этом живая: она ждёт клавиатуру.
                        ? "Клавиатуры Apple сейчас нет, и приходит ли с неё Eject — неизвестно. " +
                          "Настройка сработает на алюминиевых моделях по USB: они её присылают."
        ...
        private void AddModifierCards(StackPanel stack)
        {
            ...
            presets.Children.Add(PresetButton("Без изменений", "Модификаторы — как приходят от клавиатуры", delegate
            {
        ...
        private UIElement RightOptionCard()
        {
            ...
            string what =
                now == "fn"
                    ? (YieldingNow
                        ? (Models.FunctionKeyCount() > 12
                            ? "Драйвер Apple занимается только F1–F12, а F13 и дальше переключает " +
                              "эта клавиша. "
        ...
        private UIElement PageLayout()
        {
            ...
            if (_s.Physical != PhysLayout.Auto)
                // И здесь ANSI не выдаём за распознанное: признак заведён им же и меняется
                // только на ISO и JIS. Человек с европейской клавиатурой, выставивший ISO
                // руками, читал, что «распознано ANSI», — и решал, что распознавание
                // сломано, или возвращал «Определять самой» и получал две перепутанные
                // клавиши обратно.
                physNote += "Так выбрано в списке ниже, и выбор человека главнее всего " +
                            "остального: ни распознанное по нажатиям, ни записанное в модели " +
                            "не учитывается. " +
                            (detected == PhysLayout.Ansi
                                ? "По нажатиям, впрочем, пока не встретилось ни ISO, ни JIS."
                                : "По нажатиям распознано: " + Models.PhysName(detected) + ".");
                ...
                physNote += "У Magic Keyboard исполнение не закодировано в идентификаторе, поэтому оно " +
                            // Слева от «1», а не от «Z»: на ISO-клавиатуре Apple эти две
                            // клавиши переставлены аппаратно, и скан-код 0x56, по которому
                            // узнаётся ISO, приходит именно слева от «1». Замерено, записано
                            // в ARCHITECTURE.md. Совет нажать клавишу слева от «Z» не мог
                            // сработать ни разу: она шлёт 0x29, который не значит ничего.
                            "определяется по нажатиям: клавиша слева от «1» выдаёт ISO, клавиши у пробела — JIS. " +
                            ...
                            (detected == PhysLayout.Ansi
                                ? "Ни ISO, ни JIS по нажатиям пока не встретилось, поэтому " +
                                  "клавиатура считается ANSI. Нажмите клавишу слева от «1» — " +
            ...
            stack.Children.Add(Card("Раскладки Apple", null,
                ...
                Note("Apple раскладывает буквы и знаки иначе, чем Microsoft, — и именно поэтому " +
                     ...
                     "клавиши (´ ` ¨ ˆ ~) работают.\n\n" +
                     // Только когда третий уровень включён: с завода он выключен нарочно,
                     // и обещать набор символов, которых не набрать ничем, — врать.
                     (_s.OptLevel == OptLevel.Off
                        ? "Символы, напечатанные на клавишах третьими, сейчас набирать нечем: " +
                          "третий уровень выключен. Включается он карточкой «Правый ⌥» " +
                          "на странице «Клавиши»."
        ...
        /// <summary>Сколько медиакодов верхнего ряда замечено — без клавиши ⏏.</summary>
        private static int RowUsages()
        {
            int n = 0;
            foreach (int u in KeyWatch.AllUsages()) if (u != Native.UsageEject) n++;
            return n;
        }
        ...
        private void AddDeviceCards(StackPanel stack)
        {
            ...
            else
            {
                stack.Children.Add(Card("Заряд батареи", null,
                    // «Эта клавиатура» — только когда она есть. Пустой путь к источнику
                    // значит и «у неё нет вендорной коллекции», и «клавиатуры Apple нет
                    // вовсе», а карточка стоит на странице безусловно: рядом с надписью
                    // «Клавиатуры не найдены» она уверяла, что заряд не отдаёт клавиатура,
                    // которой нет, — и человек переставал ждать на клавиатуре, которая
                    // заряд отдаёт прекрасно.
                    Note(battery == KeyboardBattery.NoSource && !Devices.AppleConnected
                        ? "Клавиатуры Apple сейчас нет — спрашивать заряд не у кого. " +
                          "Подключите её, и число появится здесь само."
                        : battery == KeyboardBattery.NoSource
            ...
            // две строки, а по Bluetooth это секунды. На потоке окна Windows успевала
        ...
        /// <summary>
        /// Оговорка о том, что ⌘ и control поменялись местами. Пусто — не поменялись.
        ///
        /// Спрашиваем ровно тем же способом, каким спрашивает перехват: во что
        /// превращается зажатая клавиша. CmdLivesOn отвечает на половину вопроса —
        /// «⌘ увели», — а здесь нужна и вторая половина: куда именно, и что при этом
        /// достанется поиску.
        /// </summary>
        private string SpaceRolesNote()
        {
            // Спрашиваем ровно про ту роль, о которой спрашивает список выше, — и только
            // про неё. Прежде оговорка выводилась из обеих ролей разом, и «Caps Lock как
            // control» — самое частое переназначение с мака, роли ⌘ не касающееся вовсе, —
            // заставляло карточку спорить с собой: список показывал «⌘ + пробел», а сноска
            // под ним сообщала, что поиск открывает ⌘ command (левая), «а не та клавиша,
            // что подписана выше».
            bool cmd = _s.SpaceSearch == Settings.SpaceCmd;

            // Своя клавиша роль не потеряла — говорить не о чем, кто бы ещё её ни играл.
            // Добавить control на Caps Lock не значит отобрать его у настоящего control.
            // Правого control здесь нет по той же причине, что и в списке ниже:
            // на клавиатурах Apple его не бывает, а TargetOf для него всегда отвечает
            // им же — то есть «своя клавиша роль не потеряла» выходило истинным всегда,
            // сколько бы control ни увели, и вся ветка control была мертва.
            ModKey[] own = cmd ? new[] { ModKey.LWin, ModKey.RWin }
                               : new[] { ModKey.LCtrl };
            foreach (ModKey k in own) if (Plays(_s.TargetOf(k), cmd)) return "";

            // Правого control в списке нет: на клавиатурах Apple его не бывает, а
            // TargetOf для него всегда отвечает им же — и он глушил настоящие случаи.
            ModKey[] all =
            {
                ModKey.CapsLock, ModKey.LCtrl, ModKey.LAlt, ModKey.RAlt,
                ModKey.LWin, ModKey.RWin
            ...
            string who = null;
            for (int i = 0; i < all.Length && who == null; i++)
                if (Plays(_s.TargetOf(all[i]), cmd)) who = titles[i];

            if (who == null)
                return "\n\nОсторожно: ни одна клавиша сейчас не играет эту роль — " +
                       "поиск открывать нечем.";
            return "\n\nЭту роль сейчас играет другая клавиша: поиск открывает " + who + ".";
        }

        /// <summary>Играет ли это назначение роль ⌘ (клавиша Windows) или control.</summary>
        private static bool Plays(ModKey target, bool cmd)
        {
            return cmd ? (target == ModKey.LWin || target == ModKey.RWin)
                       : (target == ModKey.LCtrl || target == ModKey.RCtrl);
        }
            ...
            stack.Children.Add(Card("Поиск и язык", null,
                ...
                Note(_s.SpaceSearch == Settings.SpaceNone
                        ...
                        : "Второй клавише достаётся переключение языка — через собственный " +
                          ...
                          // Роли перехват раздаёт по назначению клавиши, а не по надписи
                          // на ней: спрашивает, во что превращается зажатая. После схемы
                          // «⌘ работает как Ctrl» ⌘ приходит в Windows клавишей Ctrl —
                          // и поиск открывается по control+пробел, а ⌘+пробел переключает
                          // язык. Карточка показывала «⌘ + пробел» и молчала об этом,
                          // тогда как соседняя про ту же беду говорит.
                          SpaceRolesNote())));
        ...
        private UIElement StateCard()
        {
            ...
            // а список выше показывает настройку, а не действительность. Прежде об этом
            // говорила только мелкая подпись в подвале да подсказка значка.
            if (_engine != null && !String.IsNullOrEmpty(_engine.Failure))
                lines.Children.Add(Note(_engine.Failure));

            if (!String.IsNullOrEmpty(KeyWatch.Failure))
                lines.Children.Add(Note(KeyWatch.Failure + " " +
                                        // «Работают» — только когда работают. Обе беды
                                        // приходят вместе чаще, чем поодиночке: у них
                                        // общая причина — программа перестала успевать.
                                        // Соседние строки одной карточки говорили
                                        // противоположное, и человек верил второй.
                                        // «Переназначения работают» — только когда они
                                        // и правда работают: про них отвечает список
                                        // в этой же карточке, и ответ у него бывает
                                        // «Выключены». Карточка спорила сама с собой,
                                        // а подсказка значка в тот же миг говорила верно.
                                        ((_engine != null && !String.IsNullOrEmpty(_engine.Failure))
                                            || !_s.Enabled
                                            || (_s.PauseWhenAppleAbsent && !Devices.AppleConnected)
                                            ? "Заодно перестало работать всё, что узнаётся "
                                            : "Переназначения работают, а вот всё, что узнаётся ") +
                                        "по нажатиям: исполнение клавиатуры, верхние F-клавиши, " +
                                        "клавиша Eject, страница «Диагностика» и яркость от кодов " +
                                        // Чужого драйвера, а не Apple: тот этих кодов не шлёт
                                        // вовсе — замерено, коллекция COL04 при нажатии F1 молчит.
                                        "чужого драйвера, забравшего верхний ряд. И самое дорогое: " +
                                        "программа не увидит медиакодов, а значит не уступит " +
                                        "верхний ряд драйверу Apple ни при каких настройках — " +
                                        "F1–F12 будут переназначаться дважды." +
                                        // Про перезапуск говорит первая надпись, если она есть:
                                        // повторять один и тот же совет дважды подряд незачем.
                                        (_engine != null && !String.IsNullOrEmpty(_engine.Failure)
        ...
        /// <summary>
        /// Можно ли странице проверки слушать нажатия: окно видно и оно впереди.
        ///
        /// Шов ровно для стенда — третий на всю программу, рядом с Input.Sink
        /// и Engine.HeldProbe. Стенд окна не показывает нарочно (оно выскочило бы
        /// на экран человеку), а проверять надо обе стороны: и что спрятанное окно
        /// не подписывается, и что показанное подписывается.
        /// </summary>
        internal static Func<bool> ShownProbe { get; set; }

        private bool SelfTestMayListen
        {
            get
            {
                Func<bool> probe = ShownProbe;
                return probe != null ? probe() : (IsVisible && IsActive);
            }
        }

        /// <summary>
        /// Снять подписку и вдобавок забыть накопленное. Разделено с DetachSelfTest
        /// нарочно: пересборка той же страницы — не уход с глаз. Стирать набранное
        /// на каждой пересборке значило бы терять список от любого чужого события —
        /// пришедшего заряда, сменившейся темы.
        /// </summary>
        ...
        private void AddSelfTestCards(StackPanel stack)
        {
            ...
            stack.Children.Add(Card("Как это читать", null,
                Note("Нажимайте клавиши по одной — ниже появится, что именно дошло до Windows. " +
                     ...
                     (String.IsNullOrEmpty(KeyWatch.Failure)
                        ...
                        // Перепись оборвалась — строк не будет ни на одном нажатии, и говорить
                        // тут «это не поломка» значит отправить человека искать причину
                        // в клавиатуре и в драйвере, тогда как чинится это перезапуском.
                        : "Сейчас страница не покажет ничего ни на одном нажатии: " +
                          KeyWatch.Failure + " Помогает перезапуск программы.") +
                     (yielding
            ...
            var reset = new Button { Content = "Забыть всё замеченное", HorizontalAlignment = HorizontalAlignment.Left };
            reset.SetResourceReference(StyleProperty, "Btn");
            reset.Click += delegate
            {
                KeyWatch.Forget();
                _selfTestLines.Clear();
                // Перестраиваем страницу целиком, а не только список: забвение меняет
                // и то, что показано на других страницах, — исполнение клавиатуры,
                // счёт F-клавиш, признак ⏏. Обновив один список, окно ещё какое-то
                // время показывало прежнее.
                BuildPage();
            };
            stack.Children.Add(Card("Забыть замеченное", null,
                Note("Кнопка стирает не только список выше. Вместе с ним забывается всё, " +
                     "что программа узнала по нажатиям: исполнение клавиатуры (ISO/JIS), " +
                     "сколько у неё F-клавиш, приходила ли Eject и видела ли программа " +
                     "медиакоды — а по ним она решает, уступать ли верхний ряд драйверу." +
                     // Про исполнение — только когда его правда определяют по нажатиям.
                     // Выбранное человеком и записанное в модели забвение не трогает,
                     // а предупреждение обещало разъехавшиеся клавиши всем подряд.
                     (_s.Physical == PhysLayout.Auto
                          && (Devices.AppleModel == null || Devices.AppleModel.Phys == PhysLayout.Auto)
                        ? "\n\nПока исполнение не узнано заново, программа считает клавиатуру " +
                          "ANSI — а значит, на европейской две клавиши снова встанут не на свои " +
                          "места. Вернуть их сразу можно, нажав клавишу слева от «1», или выбрав " +
                          "исполнение вручную на странице «Раскладка»."
                        : "\n\nИсполнение клавиатуры при этом не изменится: оно у вас задано " +
                          "не по нажатиям.")),
                reset));

            // Подписываемся, только когда окно видно и в фокусе. Защита стояла на одних
            // событиях — «спрятали», «свернули», «потеряли фокус», — а страницу
            // пересобирают и мимо них: ответ о заряде приходит раз в минуту сам, щелчок
            // по значку в трее зовёт пересборку у спрятанного окна, смена темы Windows —
            // тоже. Подписка возвращалась к невидимому окну, и в нём копилось всё, что
            // человек печатает в другом, — при том что карточка обещает обратное,
            // а снять её было уже нечем: ни «потеря фокуса», ни «спрятали» больше
            // не придут.
            if (SelfTestMayListen)
            {
        ...
        private UIElement PageDriver()
        {
            ...
            if (installed)
            {
                ...
                // И когда значения нет — тоже строкой, а не молчанием. Прежде строка
                // просто пропадала, обе кнопки режима стояли без отметки «сейчас так»,
                // и понять, какой режим программа считает действующим, было нечем.
                status.Children.Add(KeyValue("Верхний ряд у драйвера",
                    fb == 1 ? "медиа сразу, как на маке"
                    : fb == 0 ? "F-клавиши сразу"
                    : "в реестре не задано — считаем «медиа сразу», как принято у Apple"));
                ...
                status.Children.Add(KeyValue("Преобразует ряд на деле",
                    KeyWatch.MediaSeen
                        // Считаем только ряд: ⏏ в признак «драйвер преобразует ряд»
                        // не входит нарочно, и разбавлять им единственное доказательство
                        // работы драйвера незачем.
                        ? "да — медиакодов верхнего ряда замечено: " + RowUsages()
                        // Верхнего ряда: ⏏ в этот признак не считается нарочно, и «ни одного
                        // медиакода» спорило бы со страницей «Диагностика», где ⏏ сосчитана.
                        : "не замечено ни одного медиакода верхнего ряда"));
            }
            stack.Children.Add(Card("Что найдено", null, status));

            // Итог последнего действия — до всего, что зависит от «установлен ли он».
            // Прежде эта строка жила внутри блока установленного драйвера: удача
            // удаления переводила Installed в ложь, и человек не видел ни «Драйвер
            // удалён», ни требования перезагрузиться — а неудача отчитывалась исправно.
            string driverSay = _driverBusy ? _driverText : _fnNotice;
            if (!String.IsNullOrEmpty(driverSay))
            {
                // Пока работа идёт — это не последнее действие, а текущее.
                stack.Children.Add(Card(_driverBusy ? "Сейчас" : "Последнее действие",
                                        null, Note(driverSay)));
            }
            ...
            if (installed)
            {
                ...
                modes.Children.Add(PresetButton(
                    ...
                    // Замерено: с драйвером в этом режиме F1 и F2 не дают ничего — он их
                    // съедает и наружу не выпускает. Кнопка обещала обратное: «яркость
                    // остаётся, программа ловит коды и отдаёт мониторам». Человек
                    // нажимал её и терял управление яркостью внешних мониторов,
                    // не понимая, отчего.
                    "Яркость внешних мониторов при этом пропадёт: F1 и F2 драйвер забирает " +
                    "себе и наружу не выпускает — это замерено. Вернуть её можно только " +
                    "второй кнопкой.",
                ...
                modes.Children.Add(PresetButton(
                    ...
                    // Яркость даёт не режим драйвера, а собственный режим программы:
                    // до неё F1 доходит, а даст ли она яркость — решает «Основной режим»
                    // на странице «Клавиши». Обещать яркость одним лишь режимом драйвера
                    // значило звать человека переключить его и не получить ничего.
                    (_s.MediaFirst || _s.FnSubstitute != ModKey.None
                        ? " Яркостью внешних мониторов занимается программа по DDC/CI, " +
                          "пока переназначения включены."
                        : " Яркости при этом не будет: на странице «Клавиши» выбран режим " +
                          "«F-клавиши сразу» и нет заменителя Fn, так что F1 и F2 уходят " +
                          "в программы обычными F-клавишами."),
                ...
                if (AppleDriver.TakesFunctionRow && !KeyWatch.MediaSeen)
                {
                    ...
                    // «В реестре сказано» — только когда там правда сказано. Значение −1
                    // означает, что OSXFnBehavior не нашёлся нигде, и режим «медиа сразу»
                    // мы берём по умолчанию Apple, то есть по догадке. Ссылаться на реестр
                    // в этом случае — выдавать собственное предположение за прочитанное:
                    // человек лез в regedit и не находил ни значения, ни пути к нему.
                    string said = AppleDriver.FnBehavior >= 0
                        ? "В реестре сказано, что медиаклавиши делает драйвер"
                        : "Значения OSXFnBehavior в реестре нет, и программа считает, что "
                          + "драйвер работает так, как принято у Apple, — медиаклавишами сразу";
                    stack.Children.Add(blind
                        ? Card("Проверить, забирает ли драйвер ряд, сейчас нечем", null,
                            Note(said + ". Так ли это " +
                                 "на деле, программа узнаёт по приходящим медиакодам — а их она " +
                                 "сейчас не видит: " + KeyWatch.Failure + "\n\n" +
                                 "Пока это так, программа ряд драйверу не уступает: она берёт " +
                                 "F1–F12 на себя. Если драйвер их тоже забирает, верхний ряд " +
                                 "может не работать вовсе — до перезапуска программы."))
                        : Card("Драйвер настроен забирать ряд, но пока молчит", null,
                            Note(said + ", однако ни одного " +
                                 ...
                                 "Пока это так, программа ряд драйверу не уступает: она берёт " +
                                 "F1–F12 на себя, чтобы они не оказались мёртвыми. Как только придёт " +
                                 "первый медиакод, она снова уступит.\n\n" +
                ...
                stack.Children.Add(Card("Кто занимается функциональным рядом", null,
                    Note("Драйвер даёт медиаклавиши и настоящую Fn, но переназначать верхний " +
                         ...
                         "Яркость внешних мониторов требует трёх вещей разом: чтобы F1 и F2 " +
                         "дошли до программы — то есть режим драйвера «F-клавиши сразу» или " +
                         "драйвера нет вовсе; чтобы «Основной режим» на странице «Клавиши» " +
                         "стоял на медиафункциях либо был зажат заменитель Fn; и чтобы на самих " +
                         "F1 и F2 стояла яркость — их назначение там же и меняется. Драйвер " +
                         "в режиме «медиа сразу» F1 и F2 забирает себе и наружу не выпускает."),
                    modes));
            }
        ...
        private UIElement PageAbout()
        {
            ...
            if (_s.DeveloperMode)
                column.Children.Add(Card("Режим разработчика", "Включён",
                    ...
                    Note("Открывает «Диагностику», а на других страницах — номера языков ввода " +
                         "и путь к настройке OSXFnBehavior в реестре. Исполнение клавиатуры " +
                         "выбирается и без режима: настройка живая, и прятать её значило бы " +
                         "оставить её без хозяина.")));
        ...
        {
            // Тот же вопрос, что и у StartSetup, и по той же причине: погашенными эти
            // кнопки становятся только на сборке страницы, а установка из указанной
            // папки страницу не пересобирает вовсе. Без проверки щелчок поднимал
            // reg.exe с запросом прав администратора поверх уже висящего запроса
            // от pnputil — два UAC один поверх другого.
            if (SetupBusy) { SetupSay("Уже идёт работа, дождитесь конца."); return; }
            ...
            ThreadPool.QueueUserWorkItem(delegate
            {
                ...
                ToWindow(delegate
                {
                    ...
                    _fnNotice = ok
                        ...
                        : "Не вышло: " + error;
                    // _apply() здесь незачем: запись меняет чужой ключ реестра, а не настройки.
            // Уступку ряда решают AppleDriver.TakesFunctionRow и KeyWatch.MediaSeen —
            // живые признаки, снимок настроек к ним отношения не имеет.
                    BuildPage();
                });
            });
        }
        ...
        private void ApplyDriverProfile()
        {
            ...
            if (YieldingNow)
            {
                // Только когда подменять правда нечем. Драйвер забирает ровно первые
                // двенадцать, и на клавиатуре с F13–F19 заменителем переключают именно
                // их: сняв его, кнопка делала эти назначения мёртвыми и объясняла это
                // тем, что «подменять больше нечем», — споря с соседней карточкой.
                if (_s.FnSubstitute != ModKey.None && Models.FunctionKeyCount() <= 12)
                {
                    _s.FnSubstitute = ModKey.None;
