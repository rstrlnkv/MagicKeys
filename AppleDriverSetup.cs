        ...
        public static string FetchSevenZip(Action<double, string> report, out string error)
        {
            ...
            try
            {
                ...
                if (!String.Equals(now, SevenZipMsiSha256, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // Отпускаем файл прежде, чем удалять: пока он открыт без права
                    // на удаление, File.Delete отказывает всегда, и несовпавший пакет
                    // оставался лежать — хотя код явно хотел его убрать. Повторное
                    // закрытие из using безвредно.
                    keep.Dispose();
