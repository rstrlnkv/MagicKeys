    ...
    internal static class Updater
    {
        ...
        public static Release Check(string channel, out string error)
        {
            ...
            catch (WebException ex)
            {
                ...
                bool answered = http != null && http.StatusCode == HttpStatusCode.NotFound;
                error = answered
                    ? "GitHub не знает такого репозитория — проверять обновления сейчас нечем"
                    : "не удалось связаться с GitHub";
                // Ответ получен — значит проверка состоялась, и отметку времени ставим:
                // иначе окно вечно писало «Обновления ещё не проверялись».
                if (answered) Checked = true;
                return null;
            }
            catch
            {
