...
namespace MagicKeys
{
    ...
    internal static class Updater
    {
        ...
        public static Release Check(string channel, out string error)
        {
            ...
            {
                // 404 по этому адресу значит «нет такого репозитория»: список выпусков
                // при их отсутствии отдаётся кодом 200 и пустым массивом. Переименовали,
                // скрыли или удалили — говорить «выпусков пока нет» тут значит успокоить
                // человека там, где проверять обновления больше нечем.
                using (WebResponse raw = ex.Response)
                {
                    var http = raw as HttpWebResponse;
                    // Любой ответ — это состоявшаяся проверка, а не обрыв связи. 403 здесь
                    // не выдумка: это исчерпанный предел анонимных запросов к api.github.com
                    // (шестьдесят в час на адрес — общий выход в офисе, VPN, домашний
                    // роутер). Человек читал «не удалось связаться» и шёл чинить исправную
                    // сеть. А 404 по этому адресу значит «нет такого репозитория»: список
                    // выпусков при их отсутствии отдаётся кодом 200 и пустым массивом.
                    if (http == null) { error = "не удалось связаться с GitHub"; return null; }

                    int code = (int)http.StatusCode;
                    if (code == 404)
                        error = "GitHub не знает такого репозитория — проверять обновления сейчас нечем";
                    else if (code == 403 || code == 429)
                        error = "GitHub сейчас не отвечает на запросы с этого адреса — "
                              + "предел бесплатных проверок исчерпан, попробуйте через час";
                    else
                        error = "GitHub ответил кодом " + code;
                    // Проверка состоялась — иначе окно вечно писало «Обновления ещё
                    // не проверялись».
                    Checked = true;
                }
                return null;
        ...
        private static string SignatureProblem(string path)
        {
                ...
                verdict != Native.CERT_E_EXPIRED)
            {
                ...
                if (verdict == CheckFailed)
                    return "проверить подпись установщика не удалось — попробуйте позже";
        ...
        private static int Verify(string path)
        {
            ...
            // Не смогли проверить — считаем, что не прошло. Но своим кодом, а не чужим:
            // «у установщика нет подписи» отправляло скачивать выпуск заново там, где
            // упала сама проверка, и повторная закачка не помогала ничем.
        ...
        /// <summary>
        /// «Проверка сорвалась» — не вердикт Windows, а наш собственный. Взят из
        /// свободного участка кодов, чтобы не спутаться ни с одним настоящим.
        /// </summary>
        private const int CheckFailed = unchecked((int)0x8A150001);
