...
/// Спросить у Windows нельзя, поэтому судим косвенно: сырой ввод видел нажатие
        ...
        private int HandleLayout(Settings s, Native.KBDLLHOOKSTRUCT k, bool down)
        {
            ...
            // Клавишу, которую мы уже держим, обратно не отдаём. Пять отступлений
            // ниже — зажатый control или Win, пропавшая раскладка, Alt не третьим
            // уровнем, пустой уровень, «не отличается от Microsoft» — на автоповторе
        ...
        private void SubstituteRelease()
        {
            ...
            if (_subReleased++ == 0 && vk != 0)
            {
                if (IsMenuKey(vk)) Input.Tap(VkNoop);
                Input.Key(vk, false);
                // И забываем о нём на это время. Запись — ответ на вопрос «что Windows
                // держит СЕЙЧАС», и всё время навигации она отвечала неправду: аккорд,
                // нажатый поверх начатой навигации, «возвращал» снятый control — и
                // следующая стрелка уходила Ctrl+Page Up, то есть на соседнюю вкладку.
                if (_fnPhys != ModKey.None) _injected.Remove(_fnPhys);
            }
        }
        ...
        private void SubstituteRestore()
        {
            if (--_subReleased <= 0)
            {
                ...
                {
                    Input.Key(vk, true);
                    if (_fnPhys != ModKey.None) _injected[_fnPhys] = vk;
                    if (IsMenuKey(vk)) Input.Tap(VkNoop);
                }
            }
        }
