...
namespace MagicKeys
{
    ...
    public sealed class Settings
    {
        ...
        /// <summary>
        /// Достижим ли этот модификатор хоть какой-нибудь клавишей.
        ///
        /// Спрашивать надо именно так. Перехват узнаёт ⌥ не по надписи, а перебором
        /// зажатых клавиш с вопросом «во что она превращается»: третий уровень работает
        /// и тогда, когда левый Alt живёт на Caps Lock. Правило, смотревшее на одно поле
        /// MapLAlt, резало настройку в работающей раскладке.
        /// </summary>
        public bool Reaches(ModKey target)
        {
            ModKey[] all =
            {
            ...
            foreach (ModKey phys in all) if (TargetOf(phys) == target) return true;
            return false;
        }

        /// <summary>
        /// Привести настройки в согласие сами с собой.
        ///
        /// Зовут отсюда двое: чтение файла и всякая правка из окна. Файл — единственное
        /// место, откуда приходят значения, которых окно выбрать не даёт: он остался
        /// от прежней версии, его правили руками, запись оборвалась. А окно правит
        /// по полю за раз, и два поля по отдельности законные вместе дают бессмыслицу.
        /// Свести их можно только здесь: список в окне, не найдя своего значения, молча
        /// показывает первый пункт — то есть врёт.
        ///
        /// Правило, которое отменяет выбор человека, обязано быть видно в окне: страница
        /// после правки пересобирается, и переключатель, чей выбор здесь отменили, должен
        /// погаснуть сам. Молча отменённый выбор — это окно, показывающее одно и держащее
        /// другое.
        /// </summary>
        public void Normalize()
        {
            ...
            // он отдаёт «оставить как есть». Проверка на null была мёртвой, и файл
            // от прежней версии с переименованным действием проходил нетронутым: список
            // в окне показывал первый пункт, а в файле продолжал лежать прежний код
            // и переписывался при каждом сохранении.
            for (int i = 0; i < FKeys.Length; i++)
                if (Actions.Get(FKeys[i]).Id != FKeys[i]) FKeys[i] = "none";
            if (Actions.Get(NumpadClear).Id != NumpadClear) NumpadClear = "none";
            if (Actions.Get(EjectKey).Id != EjectKey) EjectKey = "none";
            ...
            // превращается, так что «правый Alt на Caps Lock» его устраивает. Спрашиваем
            // достижимость, а не поле: прежнее правило резало настройку в раскладке,
            // где она прекрасно работала.
            if (OptLevel == OptLevel.RightOption && !Reaches(ModKey.RAlt)) OptLevel = OptLevel.Off;

            // «Любой ⌥» — это прежде всего про левый: правый и так работает уровнем ниже.
            // Не достать левый Alt ничем — значит обещание перевёрнуто: меню открывать
            // он не перестал, а символов не даёт. Опускаем до правого, если тот достижим.
            if (OptLevel == OptLevel.AnyOption && !Reaches(ModKey.LAlt))
                OptLevel = Reaches(ModKey.RAlt) ? OptLevel.RightOption : OptLevel.Off;
            ...
            // Прежнее правило это стирало при следующем запуске, ничего не сказав, — и
            // стирало несимметрично: та же пара на левом ⌘, которую делает готовая схема
            // «⌘ работает как Ctrl», спокойно переживала перезапуск.
        }
    }
