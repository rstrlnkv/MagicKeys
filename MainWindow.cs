    ...
    internal sealed class MainWindow : Window
    {
        ...
        private UIElement PageLayout()
        {
            ...
            if (_s.Physical != PhysLayout.Auto)
                ...
                // клавиши обратно.
                physNote += "Так выбрано в списке ниже. " +
                            (detected == PhysLayout.Ansi
                                ? "По нажатиям пока не встретилось ни ISO, ни JIS."
                                : "Распознанное по нажатиям (" + Models.PhysName(detected) + ")") +
                            (detected == PhysLayout.Ansi ? " Это" : " и записанное в модели при этом") +
                            " не учитывается: выбор человека главнее.";
        ...
        {
            // Кто приходит в Windows клавишей Windows — тот и играет роль ⌘.
            string cmdKey = null, ctrlKey = null;
            ModKey[] all =
            {
                ModKey.CapsLock, ModKey.LCtrl, ModKey.RCtrl, ModKey.LAlt, ModKey.RAlt,
                ModKey.LWin, ModKey.RWin
            };
            string[] titles =
            {
                "Caps Lock", "control (левый)", "control (правый)",
                "⌥ option (левый)", "⌥ option (правый)",
                "⌘ command (левая)", "⌘ command (правая)"
            };
            for (int i = 0; i < all.Length; i++)
            {
                ModKey t = _s.TargetOf(all[i]);
                if ((t == ModKey.LWin || t == ModKey.RWin) && cmdKey == null) cmdKey = titles[i];
                if ((t == ModKey.LCtrl || t == ModKey.RCtrl) && ctrlKey == null) ctrlKey = titles[i];
            }

            bool cmdOwn = cmdKey == null || cmdKey.StartsWith("⌘");
            bool ctrlOwn = ctrlKey == null || ctrlKey.StartsWith("control");
            if (cmdOwn && ctrlOwn) return "";

            string which = _s.SpaceSearch == Settings.SpaceCmd ? cmdKey : ctrlKey;
            if (which == null)
                return "\n\nОсторожно: клавиши, которая играла бы эту роль, сейчас нет вовсе — " +
                       "поиск открывать нечем.";
            return "\n\nПереназначения увели эти роли на другие клавиши: поиск открывает " +
                   which + ", а не та клавиша, что подписана выше.";
        }
        ...
        private void AddSelfTestCards(StackPanel stack)
        {
            ...
            stack.Children.Add(Card("Забыть замеченное", null,
                Note("Кнопка стирает не только список выше. Вместе с ним забывается всё, " +
                     ...
                     "сколько у неё F-клавиш и приходила ли ⏏.\n\n" +
                     "Пока исполнение не узнано заново, программа считает клавиатуру ANSI — " +
                     "а значит, на европейской две клавиши снова встанут не на свои места. " +
                     "Вернуть их сразу можно, нажав клавишу слева от «1», или выбрав " +
        ...
        private UIElement PageDriver()
        {
            ...
            if (installed)
            {
                ...
                modes.Children.Add(PresetButton(
                    ...
                    "яркостью внешних мониторов занимается программа по DDC/CI, пока " +
                    "переназначения включены.",
                ...
                stack.Children.Add(Card("Кто занимается функциональным рядом", null,
                    Note("Драйвер даёт медиаклавиши и настоящую Fn, но переназначать верхний " +
                         ...
                         "Яркость внешних мониторов работает, когда F1 и F2 доходят до программы, — " +
                         "то есть в режиме «F-клавиши сразу» и без драйвера вовсе. Драйвер " +
                         "в режиме «медиа сразу» их забирает себе и наружу не выпускает."),
                    modes));
            }
