...
namespace MagicKeys
{
    ...
    internal static class Devices
    {
        ...
        /// <summary>
        /// Сколько опросов уже прошло. По одному признаку «опрос вообще когда-то был»
        /// судить нельзя: он защёлкивается навсегда, а обстановка «клавиатурная коллекция
        /// поднялась, вендорная ещё нет» повторяется при каждом пробуждении и
        /// переподключении. Спрашивающему нужен не факт, а «прошёл ли опрос ПОСЛЕ того,
        /// как набор устройств стал нынешним».
        /// </summary>
        public static int ScanCount { get { return _scanCount; } }
        private static volatile int _scanCount;
        ...
        private static void RescanHeld()
        {
            ...
            lock (Sync)
            {
                ...
                _mark = mark;
                // Счётчик — под тем же замком, что и публикация набора. Снаружи между
                // ними оставалась щель: забвение, попавшее в неё, запоминало отметку
                // на единицу меньше, и опрос, целиком отработавший ДО забвения,
                // засчитывался как прошедший после.
                _scanned = true;
                _scanCount++;
            }
        ...
        private static List<KeyboardInfo> Enumerate(out string statusPath)
        {
            ...
            // Не совпало ни с одной — значит не знаем, и запасного ответа здесь нет.
            //
            // Запасной был, и дважды пытались его сузить: сперва «клавиатура Apple
            // где-то есть», потом «и коллекция ровно одна». Оба раза мимо: по Bluetooth
            // вендорная коллекция клавиатуры поднимается не сразу, и в этот миг рядом
            // лежащая Magic Mouse — единственная коллекция в списке — отдавала свои
            // проценты под словом «заряд клавиатуры». Число настоящее, оттого и незаметно.
            //
            // Оправдание «модель может быть незнакомой» было неверным с самого начала:
            // совпадение ищется по идентификатору изделия из пути устройства против
            // dwProductId, а каталог моделей в этом не участвует вовсе. Не знать —
            // честнее: для этого и заведено состояние «спрашивать некого».
