...
namespace MagicKeys
{
    ...
    internal sealed class Engine
    {
        ...
        /// <summary>
        /// То же самое про клавиши-модификаторы: чьё нажатие ушло в приложение и кого
        /// человек ещё держит.
        ///
        /// Отдельным множеством, потому что слой модификаторов ключуется физической
        /// клавишей, а не кодом. Без него он решал «наше ли это» заново на каждом
        /// событии, и автоповтор доставался ему: нажатие ушло в приложение, отпускание
        /// съели мы — Alt оставался зажатым в Windows навсегда, и снять его было нечем.
        /// Достаточно было держать ⌥ на клавиатуре ноутбука, пока просыпается Magic
        /// Keyboard: заводская пауза «Только с Magic Keyboard» кончалась ровно между
        /// нажатием и его автоповтором.
        /// </summary>
        private readonly HashSet<ModKey> _modThrough = new HashSet<ModKey>();
        ...
        private bool Handle(int msg, Native.KBDLLHOOKSTRUCT k)
        {
            ...
            if (!s.Enabled || (s.PauseWhenAppleAbsent && !Devices.AppleConnected))
            {
                // Нажатие уходит в приложение — и до отпускания клавиша принадлежит ему.
                // Записать это надо и здесь: пауза кончается сама, между нажатием
                // и его автоповтором, а разбор ниже без записи заберёт повтор себе
                // и съест отпускание.
                LetThrough(vk, k.scanCode, ext, down);
                return false;
            }
            ...
            if (TryPhysical(vk, ext, out phys))
            {
                ...
                // а мы перевернули бы флаг столько раз, сколько пришло повторов. Работает
                // это на замеренном отставании GetKeyState внутри хука: на первом нажатии
                // он отвечает «не держит», на автоповторе — «держит». Единственное место,
                // где Held спрашивают изнутри разбора, и спрашивают нарочно.
            ...
            // Записываем это в самом конце: сюда доходит ровно то, от чего отказались
            // все слои. Но не поверх чужого владения: разбор одиночных клавиш при
            // «Оставить как есть» отвечает «не взял», а запись о себе оставляет —
            // и у ⌧ выходило два хозяина, из которых снимался только один.
            if (down) { if (!Busy(vk, k.scanCode)) _letThrough.Add(vk); }
            else _letThrough.Remove(vk);
            return false;
        }
        ...
        private bool HandleModifier(Settings s, ModKey phys, bool down)
        {
            ...
            {
                int mod = NoteFnSubstitute(s, phys, down);
                ...
                // Спрашиваем «снимали ли МЫ», а не счётчик: он растёт и у пустого вызова —
                // верхний ряд зовёт SubstituteRelease безусловно, в том числе когда
                // заменитель не держат вовсе. По счётчику выходило, что под зажатой F5
                // нажатие правого ⌥ берётся себе и никуда не посылается: ни третьего
                // уровня раскладки, ни ⌥+щелчка — Windows нажатия не видит.
                if (down && _subLifted == phys)
                {
                    _modTaken.Add(phys);
            ...
            // строкой ниже.
            // И «ушло в приложение» — тоже: отпускание уйдёт туда же, куда ушло нажатие.
            if (!down) _modThrough.Remove(phys);
        ...
        private bool HandleFunctionKey(Settings s, int index, int vk, bool down)
        {
            ...
            {
                // «Оставить как есть» — но без нашего заменителя на дороге, когда его
                // держат. При режиме «F-клавиши сразу» ветка переворачивается в медиа
                // именно им, и клавиша уходила в приложение как Alt+F5 вместо F5.
                // При заводском «медиа сразу» сюда попадают и без заменителя — тогда
                // снимать нечего, и снятие выходит пустым.
                if (down)
                {
                    if (!_fkeyDown[index]) { _fkeyDown[index] = true; SubstituteRelease(); }
                }
            ...
            if (down)
            {
                ...
                // Заменитель Fn — с дороги, как и в ветке настоящей F-клавиши. При режиме
                // «F-клавиши сразу» сюда попадают ровно тогда, когда заменитель держат:
                // ветка переворачивается им же. Назначенная на F5 клавиша Home уходила
                // под зажатым ⌥ — то есть как Alt+Home, уходом браузера на домашнюю
                // страницу; Escape — как Alt+Esc, PrintScreen снимал окно вместо экрана.
                // Аккорд и готовый символ щит ставили, а «клавиша» шла мимо него.
                if (!_fkeyDown[index]) { _fkeyDown[index] = true; SubstituteRelease(); }
                RunAction(a, repeat);
            }
            else
        ...
        /// <summary>
        /// Запомнить, что нажатие ушло в приложение своим ходом, — или забыть об этом
        /// на отпускании. Модификаторы помнятся по физической клавише, прочие по коду:
        /// так же, как их и спрашивают.
        /// </summary>
        private void LetThrough(int vk, uint scan, bool ext, bool down)
        {
            ModKey phys;
            if (TryPhysical(vk, ext, out phys))
            {
                // Заменитель Fn отслеживаем и здесь: события до разбора не доходят,
                // а он про них знать обязан.
                Settings cur = _cfg;
                if (cur != null && cur.FnSubstitute != ModKey.None && phys == cur.FnSubstitute)
                    NoteFnSubstitute(cur, phys, down);
                ...
                // Снятое запоминаем ДО того, как забыть: код, снятый раньше аккордом
                // или навигацией, Windows уже не держит, и отпускать его второй раз
                // значит слать отпускание без пары — то самое, что весь остальной
                // разбор считает дефектом.
                bool wasLifted = _modLifted.Remove(phys);
                if (_subLifted == phys) _subLifted = ModKey.None;
            ...
            // Только если у клавиши нет хозяина. Запись заводится и здесь, и в хвосте
            // разбора, а снимает её один хвост — слои, вернувшиеся раньше, до него
            // не доходят. Записанная поверх чужого владения, она переживала отпускание
            // и оставалась навсегда: Busy отвечал «занята приложением», и ⌥+← молча
            // переставало узнаваться до ближайшего сброса.
            if (Busy(vk, scan)) return;
            if (down) _letThrough.Add(vk); else _letThrough.Remove(vk);
        }

        /// <summary>
        /// Отметить нажатие или отпускание заменителя Fn. Возвращает код, который
        /// клавиша по нынешним настройкам держит, или 0.
        ///
        /// Отдельным местом, потому что событий у заменителя два пути: разбор и путь
        /// мимо него — выключатель и пауза. Прежде отметку ставил только разбор,
        /// и отпущенный на паузе заменитель оставался для программы зажатым: FnHoldsVk
        ...
        /// зажатым навсегда, и записи о нём не было ни одной — общий сброс его не снимал.
        /// </summary>
        private int NoteFnSubstitute(Settings s, ModKey phys, bool down)
        {
            _fnHeld = down;
        ...
        ///
        /// Замерено: внутри обработчика хука ответ относится к состоянию ДО разбираемого
        /// события — на первом нажатии «не держит», на отпускании «держит». Из очереди
        /// сообщений того же потока ответ верен.
        ///
        /// Поэтому перечёт зажатого спрашивает его только оттуда — сообщением, выходом
        /// потока или с чужого потока. Исключение ровно одно и нарочное: счёт регистра
        /// в Handle (Caps Lock). Ему как раз и нужно отставание: на первом нажатии ответ
        /// «не держит» — флаг переворачиваем; на автоповторе «держит» — не трогаем.
        /// Спрашивая послесобытийное состояние, мы переворачивали бы счёт столько раз,
        /// сколько пришло повторов.
        /// </summary>
        ...
        private void ReleaseEverything(Settings after, bool keepHolding)
        {
            try
            {
                ...
                // Лампочку Caps Lock перечитываем у Windows: пока перехват стоял на паузе,
                // её могли переключить мимо нас, и наш счёт регистра разошёлся бы с ней.
                // Это лампочка, а не «зажата ли клавиша», — поэтому бит 1, а не 0x8000,
                // и поэтому же шов HeldProbe сюда не годится: он отвечает про удержание.
                _capsOn = (Native.GetKeyState(Vk.Capital) & 1) != 0;
