...
namespace MagicKeys
{
    ...
    internal static class Devices
    {
        ...
        private static List<KeyboardInfo> Enumerate(out string statusPath)
        {
            ...
            // Берём первую только тогда, когда она одна: клавиатура Apple в списке
            // есть, а коллекция ровно одна — значит она её и есть. Прежде хватало
            // «клавиатура Apple где-то есть»: по Bluetooth её вендорная коллекция
            // поднимается не сразу, и рядом лежащая Magic Mouse отдавала свои проценты
            // под словом «клавиатура». Не знать — честнее: для этого и заведено
            // состояние «спрашивать некого».
            if (statusPath == null && statusPaths.Count == 1)
            {
                bool anyApple = false;
